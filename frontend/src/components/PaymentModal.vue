...
<template>
  <div class="fixed inset-0 bg-black/75 backdrop-blur-md z-50 flex items-center justify-center p-4">
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-lg p-6 sm:p-7 shadow-2xl space-y-6 animate-in fade-in zoom-in duration-200 text-slate-900 dark:text-slate-100 transition-colors">
      ...
      <!-- Payment Type Selector Grid (Faqat Naqd Pul va Plastik Karta) -->
      <div class="grid grid-cols-2 gap-3">
        <!-- CASH -->
        <button 
          @click="selectedPaymentType = 'CASH'; paidAmountInput = totalAmount"
          :class="[
            'p-4 rounded-2xl border flex items-center space-x-3.5 transition-all duration-200 cursor-pointer text-left',
        ...
        >
          <div :class="['p-3 rounded-xl shrink-0', selectedPaymentType === 'CASH' ? 'bg-amber-500 text-white shadow-md' : 'bg-slate-200 dark:bg-slate-900 text-slate-500']">
            <Banknote class="w-6 h-6" />
          </div>
          <div>
            <div class="text-sm font-bold">Naqd Pul</div>
            <div class="text-[10px] opacity-70">Mijoz bergan pul & qaytim</div>
          </div>
        </button>
        ...
        >
          <div :class="['p-3 rounded-xl shrink-0', selectedPaymentType === 'CARD' ? 'bg-blue-500 text-white shadow-md' : 'bg-slate-200 dark:bg-slate-900 text-slate-500']">
            <CreditCard class="w-6 h-6" />
          </div>
          <div>
            <div class="text-sm font-bold">Plastik Karta</div>
            <div class="text-[10px] opacity-70">Humo / Uzcard terminal</div>
          </div>
        </button>
      </div>
